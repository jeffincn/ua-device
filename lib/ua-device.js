...
module.exports = function (ua) {
    ...
    if (uaData.device.type === 'mobile' || uaData.device.type === 'tablet') {
        ...
        else if (match = ua.match(/(ZTE|Samsung|Motorola|HTC|Coolpad|Huawei|Lenovo|LG|Sony Ericsson|Oppo|TCL|Vivo|Sony|Meizu|Nokia|Micromax|INTEX AQUA|TECNO|GIONEE)/i)) {
        ...
        else if (match = ua.match(/redmi\s?(([\d\w\s]+)?(build)?)?/i)) {
            uaData.device.manufacturer = 'Xiaomi';
            uaData.device.model = 'HM-' + match[2].replace('Build', '');
        }
        ...
        else if (match = ua.match(/P5 mini|M35\d/i)){
            uaData.device.manufacturer = 'Gionee';
            uaData.device.model = match[1];
        }
        // handle LG
        ...
        // tecno
        else if (match = ua.match(/(TECNO[\s-_])(\w*)/i)) {
            uaData.device.manufacturer = 'Tecno';
            uaData.device.model = match[2];
        }

        // Micromax
        else if (match = ua.match(/(Micromax[\s-_\+])(\w*)/i)) {
            uaData.device.manufacturer = 'Micromax';
            uaData.device.model = match[2];
        }
        ...
        if (uaData.device.model) {
            uaData.device.model = uaData.device.model.replace(/-+|_+|\s+/g, ' ');
    ...
    if (uaData.device.type === 'desktop') {
         ...
         * facebook
         */
        else if (match = /facebookexternalhit/i.exec(ua)) {
            uaData.browser.name = 'Crawler';
            uaData.device.manufacturer = 'Facebook';
            uaData.device.model = 'Bot';
        }

        /**
         * WhatsApp
         */
        else if (match = /WhatsApp/i.exec(ua)) {
            uaData.browser.name = 'Crawler';
            uaData.device.manufacturer = 'WhatsApp';
            uaData.device.model = 'Bot';
        }
         ...
         * Googlebot
         */
        else if (match = /Googlebot/i.exec(ua)) {
            uaData.browser.name = 'Crawler';
            uaData.device.manufacturer = 'Google';
            uaData.device.model = 'Bot';
        }

    }
    else if (uaData.device.type === 'mobile' || uaData.device.type === 'tablet') {
        if (match = /mCent\/([\w.]+)/i.exec(ua)) {
            uaData.browser.name = 'mCent';
            uaData.browser.version = {
                original: match[1]
            };
        }
         ...
         * BaiduHD
         */
        else if (match = /BaiduHD\s+([\w.]+)/i.exec(ua)) {
